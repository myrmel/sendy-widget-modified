...
class Sendy_Widget extends WP_Widget {
			...
			echo $before_title . $title . $after_title;
                ...
                <form id="subscribe-form" onsubmit="return  validate_sendy_form()" action="<?php echo $instance['sendyurl']; ?>/subscribe" method="POST" accept-charset="utf-8">
                  <?php if($instance['hidename']!='on'){ ?>
                    ...
                    <br/>
                    <?php } ?>
                    <label class="hide" for="email">Email</label><br />
                    <div class="input-group">
	                    <input type="text" name="email" id="subscriber-email" class="form-control" placeholder="<?php echo $instance['placeholder']; ?>"/>
	                    <span class="input-group-btn">
						<input type="submit" name="sub-submit" value="<?php echo $instance['text_btn']; ?>" id="sub-submit" class="btn btn-default"/>
	                    </span>
                    </div>
	...
	public function form( $instance ) {
		...
		if ( isset( $instance[ 'placeholder' ] ) ) {
			...
			$placeholder = __( ' ', 'sendywidget' );
		}	
		if ( isset( $instance[ 'text_btn' ] ) ) {
			$text_btn = $instance[ 'text_btn'];

		}
		else {
			$text_btn = __( ' ', 'sendywidget' );
		}	
		...
		<p>
		<label for="<?php echo $this->get_field_id( 'text_btn' ); ?>"><?php _e( 'Button text:' ); ?></label>
